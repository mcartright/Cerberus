...
object SocketEx {
  ...
  /*def main(args: Array[String]) {
    ...
    val functions = List(
      fileName => { IO.cat("foobar", "foo1.txt"); fileName },
      fileName => { IO.cat("barfoo", "foo2.txt"); fileName },
      fileName => { 
    ...
    var runningJobs = Set[Future[String]]()

    functions.foreach(task => {
      val jobId = qsub.spawnJob("cerberus.JobRunner", Array(localhost, port.toString))
      println("spawned "+jobId)
      
      runningJobs = runningJobs + JobRunner.dispatch(server.accept(), JobTask(task))
    })
    ...
    runningJobs.foreach(tsk => {
      tsk.value.get match {
        case Failure(msg) => println("task failed: "+msg)
        case Success(msg) => println("task succeeded "+msg)
      }
    })
